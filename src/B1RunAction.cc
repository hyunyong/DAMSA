...
{ 
  ...
  eAxis.push_back(0.1); 
  for (double i = 1.0; i < 2E3; i += 1.0){eAxis.push_back(i);}
  ...
  for (double i =   0.1; i < 30; i += 0.1){edAxis.push_back(i);}
  ...
  analysisManager->CreateH2("t_electron_et", "electron E [MeV] (1 Mev to 10 GeV)  Theta [rad] in target", peAxis, tAxis);     //7
  ...
  analysisManager->CreateH2("t_photon_et", "photon E [MeV] (1 Mev to 10 GeV)  Theta [rad] in target", peAxis, tAxis);      //11
  ...
  analysisManager->CreateH1("w_all_ed", "Energy Deposit [MeV]",5,0,5); //20
