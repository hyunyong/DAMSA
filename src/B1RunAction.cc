...
{ 
  ...
  for (G4double i = 0.0; i < 0.301; i += 0.001){eAxis.push_back(i);}
  std::vector<G4double> tAxis;
  for (G4double i = 0.0; i < 1.571; i += 1E-3){tAxis.push_back(i);}

  const B1DetectorConstruction* detectorConstruction
   = static_cast<const B1DetectorConstruction*>
     (G4RunManager::GetRunManager()->GetUserDetectorConstruction());
  G4double targetL = detectorConstruction->GetTargetLength()/cm;
  G4double detL = detectorConstruction->GetDetectorLength()/cm;



  G4cout << "[RunAct] target tickness: " << targetL << " cm" << G4endl;
  ...
  analysisManager->CreateH2("d_all_2D_ed", "Energy Deposit [MeV]", int(detL),targetL+vacL,targetL+vacL+detL, 12,-6,6); //4
  analysisManager->CreateH2("d_all_2D_edt", "Energy Deposit [MeV] by time [ns]", int(detL),targetL+vacL,targetL+vacL+detL, 3000,0,3000); //5
  ...
  analysisManager->CreateH2("w_electron_ke", "leakage e- energy [GeV]", 300,0.0,0.3, 157,0,1.57); //11
  analysisManager->CreateH2("w_positron_ke", "leakage e+ energy [GeV]", 300,0.0,0.3, 157,0,1.57); //12
  analysisManager->CreateH2("w_photon_ke", "leakage photon energy [GeV]", 300,0.0,0.3, 157,0,1.57); //13
  analysisManager->CreateH2("w_neutron_ke", "leakage neutron energy [GeV]", 300,0.0,0.3, 157,0,1.57); //14
  analysisManager->CreateH2("w_proton_ke", "leakage proton energy [GeV]", 300,0.0,0.3, 157,0,1.57); //15
  analysisManager->CreateH2("wall_all_ed", "wall energy deposit [GeV]", 600,-300,300, 600,-300,300); //16

  analysisManager->CreateH1("d_all_ed", "Energy Deposit [MeV]",300,0.0,30.); //0
  ...
  //analysisManager->CreateH1("d_all_edp", "Single Crystal", 60,-6,6); //3
  analysisManager->CreateH3("d_all_3D_ed","Energy Deposit [MeV]",120,-6,6,120,-6,6,detectorBin,targetL+vacL,targetL+vacL+detL);
